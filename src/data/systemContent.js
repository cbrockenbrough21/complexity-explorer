export const systemContent = {
  ...
  reactionDiffusion: {
    ...
    forEngineers:
      "Per step, apply a discrete Laplacian to both concentration fields and integrate the Gray-Scott reaction terms. Cost is O(w×h) per step; this implementation runs multiple micro-steps per frame for visual stability.\n\nThe production path runs entirely on the GPU: two RGBA32F textures are swapped each pass (ping-pong), so the GPU never reads and writes the same texture simultaneously. Each pass dispatches the Gray-Scott fragment shader across every texel; the 8-neighbor Laplacian and reaction terms execute in parallel for all 256×256 (or larger) cells at once. A separate display fragment shader maps A−B to color and blits to the screen, also on the GPU. This yields a ~10–50× speedup over the Canvas 2D path, making 512×512 and 1024×1024 grids real-time feasible.\n\nIf WebGL 2 or the required RGBA32F extension is unavailable the system silently falls back to the CPU Canvas 2D implementation. MiniReactionDiffusion in the learning modules intentionally keeps the CPU path — the smaller grid size needs no GPU overhead, and WebGL requires exclusive canvas context ownership.",
